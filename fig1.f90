    program esER
    implicit none
    integer N,i,j,ij,ji,l,Nk,nwmax,mcs,sc,scc,al,kcc,kc1,kc2,delta
    real(kind=8) pi
    parameter(N=5000,Nk=20,pi=4*atan(1.0))
    real(kind=8) opinion(N),self(N),coop,alpha,temp,z
    real(kind=8) beta,nw,x,t,k,cp,kc,p,h,rgre,rgim,rloc(N),rloc_i,cre,cim,rls,orders,coops,pay(n),paymax
    real(kind=8) opinion_t(N),opinion_rate(N),yk(4,N),A(4),B(4),order,orderss,coopss,rlss,ri(n),rl
...
do scc=0,1
    ...
    write(filename2,'(i2)')scc
    open(10,file="rg\"//trim(adjustl(filename1))//","//trim(adjustl(filename2))//".txt")
    open(11,file="c\"//trim(adjustl(filename1))//","//trim(adjustl(filename2))//".txt")
    open(12,file="rl\"//trim(adjustl(filename1))//","//trim(adjustl(filename2))//".txt")
if (scc==0) then
    kc1=0
    kc2=50
    delta=1
    ...
    do kcc=kc1,kc2,delta
        kc=0.1d0+kcc*0.005d0
        coops=0d0
        orders=0d0
        rls=0d0
        ...
        do mcs=1,100000
            ...
            if (mod(mcs,500)==0)then
                write(*,'(F5.3, 2X, i6, 2X, F12.9, 2X, F8.6)')kc,mcs,order,coop
            endif
            if (mcs>90000) then
        ...
        write(10,"(f5.3,4X,f12.7)")kc,orders
        write(11,"(f5.3,4X,f12.7)")kc,coops
        write(12,"(f5.3,4X,f12.7)")kc,rls
    enddo
...
enddo
    ...
    do i=1,N
        ...
        opinion(i) = atan2(sin(opinion(i)), cos(opinion(i)))
    end do
